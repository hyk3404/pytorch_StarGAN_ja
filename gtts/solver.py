...
class Solver(object):
    ...
    def test(self):
        ...
        os.mkdir("{}/TM2".format(directory_name))
        input_text = input("請輸入文字(ja):")

        with tempfile.NamedTemporaryFile(delete=True) as fp:
            tts=gTTS(text=input_text, lang="ja")
            tts.save('{}.mp3'.format(fp.name))
            mp3_path = '{}.mp3'.format(fp.name)
            wav_name = 'ja.wav'
            sound = AudioSegment.from_mp3(mp3_path)
            sound.export("{}/TM2/{}".format(directory_name,wav_name), format="wav")

        mixer.init()
        mixer.music.load('{}/TM2/{}'.format(directory_name, wav_name))
        mixer.music.play(1)
        time.sleep(1)

        print("{}/TM2{}".format(directory_name,wav_name))

        # Set data loader.
        d, speaker = TestSet(directory_name).test_data(self.src_speaker)
        ...
        for target in targets:
            ...
            with torch.no_grad():
                ...
                for filename, content in d.items():
                    ...
                    print(f'[save]:{path}')
                    librosa.output.write_wav(path, wav, SAMPLE_RATE)           
                    librosa.output.write_wav("ja.wav", wav, SAMPLE_RATE) 
